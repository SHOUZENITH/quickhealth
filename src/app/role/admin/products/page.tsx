...
export default function ProductManager() {
  ...
  // --- STATE FOR EDITING ---
  const [editingProduct, setEditingProduct] = useState<any>(null); // Tracks if we are editing
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let imageUrl = editingProduct?.image_url || null; // Default to existing image
      ...
      const payload = {
        ...
        image_url: imageUrl,
        // Don't overwrite stock if not handled here
      };
                ...
                <td className="p-4 text-right">
                  <div className="flex justify-end gap-2">
                    <button 
                      ...
                      className="text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 p-2 rounded-lg transition"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-[#0f172a] rounded-2xl shadow-2xl p-6 animate-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-6 border-b border-gray-200 dark:border-white/10 pb-4">
              ...
              <button onClick={resetForm} className="p-2 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required name="name" value={formData.name} onChange={handleInputChange} className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10" placeholder="e.g. Vitamin C" />
                </div>
                <div>
                  <label className="block text-sm font-bold mb-1">Price (IDR)</label>
                  <input required type="number" name="price" value={formData.price} onChange={handleInputChange} className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10" placeholder="e.g. 50000" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-bold mb-1">Category</label>
                <select name="category" value={formData.category} onChange={handleInputChange} className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
              ...
              <div>
                ...
                <textarea required name="description" value={formData.description} onChange={handleInputChange} className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10" rows={3} placeholder="Product details..." />
              </div>
              ...
              <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-white/5 rounded-lg border border-gray-200 dark:border-white/10">
                <input type="checkbox" name="requires_prescription" checked={formData.requires_prescription} onChange={handleInputChange} className="w-5 h-5 rounded text-blue-600" />
                <label className="text-sm font-medium">Requires Doctor's Recipe (Rx)</label>
              </div>
