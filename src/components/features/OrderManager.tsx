            ...
            <div key={order.id} className="border-b border-gray-100 dark:border-white/5 last:border-0">
               ...
               >
                  <div className="flex items-center gap-4 w-full md:w-auto">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-900 dark:text-white">{order.profiles?.display_name || 'Unknown User'}</h4>
                      <p className="text-xs text-gray-500">Ordered: {new Date(order.created_at).toLocaleDateString()}</p>
                    </div>
                  </div>

                  <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                     <span className="font-bold text-gray-900 dark:text-white">{formatIDR(order.total_amount)}</span>
                 ...
                 <div className="px-6 pb-6 pt-0 pl-4 md:pl-20 bg-gray-50/50 dark:bg-black/20">
                   ...
                   {order.prescription_url && (
                      <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/10 rounded-lg border border-blue-200 dark:border-blue-500/20">
                        ...
                        <a href={order.prescription_url} target="_blank" rel="noopener noreferrer" className="text-sm underline text-blue-500">View Document</a>
                      </div>
                   )}
...
function StatCard({ icon, bg, label, value }: { icon: any, bg: string, label: string, value: number }) {
  return (
